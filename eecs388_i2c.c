...
void bufWriteSet(int led, int breakupNumber) {
   // helper function that sets 0 through 4 of bufWrite, including the breakup
   bufWrite[0] = led;
   bufWrite[1] = 0;
   bufWrite[2] = 0;
   breakup(breakupNumber, &bufWrite[3], &bufWrite[4]);
}

void steering(int angle) {
   printf("Steer to angle: ");
   printf("%i", angle);
   bufWriteSet(PCA9685_LED1_ON_L, getServoCycle(angle));
   _Bool success = metal_i2c_transfer(i2c, PCA9685_I2C_ADDRESS, bufWrite, 5, bufRead, 1);
   if(success) {
       printf("success");
   }
}
...
void stopMotor() {
   bufWriteSet(PCA9685_LED0_ON_L, 280);
   delay(2000); // 2 second delay for calibration
   metal_i2c_transfer(i2c, PCA9685_I2C_ADDRESS, bufWrite, 5, bufRead, 1);
}

void driveHelper(int speed1, int speed2, int speed3, uint8_t speedFlag){
    // function for both driveForward and driveReverse to use
    int speed = 0;
    switch(speedFlag) {
       case 2: speed = speed2; break;
       case 3: speed = speed3; break;
       default: speed = speed1; break; // case 1, speed 1 is the default speed
    }
    printf("drive at speed: ");
    printf("%i", speed);
    bufWriteSet(PCA9685_LED0_ON_L, speed);
    _Bool success = metal_i2c_transfer(i2c, PCA9685_I2C_ADDRESS, bufWrite, 5, bufRead, 1);
    if(success) {
       printf("success");
   }
...
void driveForward(uint8_t speedFlag) {
   driveHelper(303, 305, 307, speedFlag);
}

void driveReverse(uint8_t speedFlag) {
   driveHelper(257, 255, 253, speedFlag);
}
...
*/
    ...
    //*/
    stopMotor();
    printf("finished with instructions\n");
}
